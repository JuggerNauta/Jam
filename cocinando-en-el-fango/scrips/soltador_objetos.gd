...
const OBJETO_RECOGIBLE := preload("res://escenas/objetos/objeto_recogible.tscn")
...
func _on_objeto_soltado(_indice: int, objeto: ObjetoData) -> void:
	...
	instancia.color = objeto.color
	instancia.nombre = objeto.nombre
	instancia.global_position = jugador.global_position + Vector2(0, 100)
